...
const SYSTEM_PROMPT = [
  ...
  '1. Call `screenshot` first to see ALL monitors. The result includes a `monitors` array so you know each display\'s coordinate range.',
  '2. Identify the target visually. Note its pixel coordinates from the screenshot (virtual-desktop absolute — directly usable by click/move).',
