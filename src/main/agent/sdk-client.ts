...
const SYSTEM_PROMPT = [
  ...
  '   - On X11 (rare here): `shell_exec("wmctrl -l")` or `shell_exec("xdotool getactivewindow getwindowname")`.',
  '   - On KDE Wayland (this machine): `kdotool` if installed (`shell_exec("which kdotool && kdotool getactivewindow")`); otherwise rely on the screenshot.',
  '   - `screenshot` for anything visual — locating a button, reading on-screen text, confirming an action landed.',
  '   Combine signals: e.g., "is the browser open?" → ps to confirm process + screenshot to find its window position.',
