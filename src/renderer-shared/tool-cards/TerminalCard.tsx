...
// Fold quiet middles: > FOLD_THRESHOLD stdout lines keeps the first line and
// the last TAIL_KEEP, with the rest behind a click-to-unfold row.
...
export function TerminalCard({ view, compact }: { view: View; compact?: boolean }) {
  ...
  const hiddenCount = stdoutLines.length - 1 - TAIL_KEEP;
  ...
  const output = (
    <>
      {view.stdout && !folded && <pre className="whitespace-pre-wrap break-words m-0">{view.stdout}</pre>}
      {folded && (
        <>
          <pre className="whitespace-pre-wrap break-words m-0">{stdoutLines[0]}</pre>
          <button
            type="button"
            onClick={() => setUnfolded(true)}
            className="block w-full text-left text-muted/70 hover:text-muted border-y border-dashed border-border/60 my-1 py-0.5"
          >
            — {hiddenCount} quiet lines hidden —
          </button>
          <pre className="whitespace-pre-wrap break-words m-0">{stdoutLines.slice(-TAIL_KEEP).join('\n')}</pre>
        </>
      )}
